...
new function () {
	...
	var _ALPHA = "ABCDEFGHIJKLMNOPQRSTUVWXYZ",
        ...
        _BASE64_CHARSET_SAFE = _ALPHANUM + "*-", // TODO: urlsafe_b64 encode in Python: -_
    ...
    var _isInstance = function (context) {
        for (var a = 1; a < arguments.length; a++) {
			if (arguments[a] in gruft && context instanceof gruft[arguments[a]]) {
                return true;
	        }
        }
        return false;
	...
	var _is = (function (that) {
		...
		return {
			...
			which: function (object) {
				for (var type in that._is) {
					if (/^[A-Z]/.test(type) && that._is[type](object)) {
						return type;
					}
				}
			...
			instance: function (object) {
				for (var a = 1; a < arguments.length; a++) {
                    if (arguments[a] in gruft && object instanceof gruft[arguments[a]]) {
                        return true;
                    }
                }
            return false;
	...
	// TODO: memoize this
    var _dict = function () {
        var items = {}, token = "gruft.f00e6a6f9b7b4886b9872039040a0e87", 
			head = arguments[0], keys = (_is.Array(head)) ? head : arguments, k = -1;
        while (++k < keys.length) {
            if (keys[k] !== null) {
                items[keys[k]] = token;   
            }
    ...
    var _import = function () {
        ...
        for (var symbol in gruft) {
			...
			if (/^__\w+__$/.test(symbol) && _is.Object(object) && "namespace" in object) {
    ...
    /**
     * Calculate number of base64 characters to represent the given bitfield.
     */
	// TODO: memoize this
    var _resizeToBase64 = function (bits) {
        var fraction = bits / 6 * 8;
        return ((fraction >> 4) + Math.ceil((fraction - fraction >> 0.5) * 2)) << 4; 
    };
